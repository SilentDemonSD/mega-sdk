...
void Session::forget(fuse_req_t request, fuse_ino_t inode, unsigned long num)
{
...
std::string Session::nextRequest()
{
    ...
    while (true)
    {
        ...
        throw FUSEErrorF("Unable to read request from session: %s", std::strerror(-result));
    }
}
...
void Session::dispatch()
{
    ...
    if (auto request = nextRequest(); !request.empty())
        fuse_session_process(mSession.get(), request.data(), request.size(), mChannel.get());
}
...
void Session::invalidateData(MountInodeID id, off_t offset, off_t length)
{
    ...
    while (!exited())
    {
        auto result = fuse_lowlevel_notify_inval_inode(mChannel.get(), id.get(), offset, length);
...
void Session::invalidateEntry(const std::string& name, MountInodeID child, MountInodeID parent)
{
    assert(!name.empty());
    assert(mChannel);
    assert(mSession);
