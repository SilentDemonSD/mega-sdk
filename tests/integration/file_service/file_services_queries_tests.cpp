...
class FileServiceQueriesGetStorageSize: public ::testing::Test
{
protected:
    void SetUp() override
    {
    ...
    // Execute mGetStorageSize with the given parameters and return populated StorageSize.
    StorageSize getStorageSize(std::int64_t accessed, std::uint64_t targetRemainingSize)
    {
    ...
    SubsystemLogger mLogger{"FileServiceQueriesGetStorageSize"};
...
TEST_F(FileServiceQueriesGetStorageSize, empty_database_returns_zeros)
{
    auto result = getStorageSize(0, 0);
    EXPECT_EQ(result, (StorageSize{0, 0, 0, 0}));
}

// Files whose accessed time is after the cutoff are not eligible for reclamation.
TEST_F(FileServiceQueriesGetStorageSize, no_files_reclaimable_when_cutoff_before_accessed)
{
    ...
    auto result = getStorageSize(50, 0);
    EXPECT_EQ(result, (StorageSize{0, 1000, 800, 2000}));
}

// A single eligible file is fully reclaimed when the target remaining size is zero.
TEST_F(FileServiceQueriesGetStorageSize, single_file_fully_reclaimed_when_target_is_zero)
{
    addFile(1, 50, 500, 400, 1000);

    auto result = getStorageSize(100, 0);
    EXPECT_EQ(result, (StorageSize{500, 500, 400, 1000}));
}
...
TEST_F(FileServiceQueriesGetStorageSize, file_not_reclaimed_when_allocated_exactly_equals_target)
{
    addFile(1, 50, 500, 400, 1000);

    // target=500 means condition is 500 > 500, which is false.
    auto result = getStorageSize(100, 500);
    EXPECT_EQ(result, (StorageSize{0, 500, 400, 1000}));
}
...
TEST_F(FileServiceQueriesGetStorageSize, partial_reclaim_reaches_exact_target)
{
    ...
    auto result = getStorageSize(100, 300);
    EXPECT_EQ(result, (StorageSize{300, 600, 600, 600}));
}

// All eligible files are reclaimed when the target remaining size is zero.
TEST_F(FileServiceQueriesGetStorageSize, all_files_reclaimed_when_target_is_zero)
{
    addFile(1, 10, 200, 200, 200);
    addFile(2, 20, 300, 300, 300);

    auto result = getStorageSize(100, 0);
    EXPECT_EQ(result, (StorageSize{500, 500, 500, 500}));
}

// Removed files are excluded from the reclaim candidate set but still counted in totals.
TEST_F(FileServiceQueriesGetStorageSize, removed_files_excluded_from_reclaim_but_included_in_totals)
{
    ...
    auto result = getStorageSize(100, 0);
    EXPECT_EQ(result, (StorageSize{200, 500, 500, 500}));
}

// Files with allocated_size == 0 are excluded from reclaim candidates.
TEST_F(FileServiceQueriesGetStorageSize, files_with_zero_allocated_size_excluded_from_reclaim)
{
    ...
    auto result = getStorageSize(100, 0);
    EXPECT_EQ(result, (StorageSize{200, 200, 200, 2000}));
}
...
TEST_F(FileServiceQueriesGetStorageSize, same_accessed_time_both_files_reclaimable)
{
    ...
    auto result = getStorageSize(100, 0);
    EXPECT_EQ(result, (StorageSize{300, 300, 300, 300}));
}
...
TEST_F(FileServiceQueriesGetStorageSize, same_accessed_time_tie_broken_by_id)
{
...
TEST_F(FileServiceQueriesGetStorageSize, oldest_files_reclaimed_first)
{
